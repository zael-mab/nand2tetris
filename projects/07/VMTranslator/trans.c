...
void    push_instruction(t_head *head, t_memory_segments *segments, t_node *inst)
{
    if (inst->op == PUSH)
    {
        // constant
...
void    arithmetic_instruction(t_node *inst, t_memory_segments *segments)
{
    if (inst->op == ADD)
    {
        dprintf(segments->fd,"//%s\n@%d\nAM=M-1\nD=M\n@%d\nAM=M-1\nD=M+D\n@%d\nA=M\nM=D\n@%d\nM=M+1\n",
...
void    trans(t_head *head, t_memory_segments *segments)
{
    ...
    while (inst)
    {
        ...
        if (!inst->arith)
        {
            push_instruction(head, segments, inst);
        }
        /////////////  /////////////////////////////////////////
        // Arithmetic
        if (inst->arith == 1)
        {
            arithmetic_instruction(inst, segments);
